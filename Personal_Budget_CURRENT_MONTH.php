    ...
    if($connection->connect_errno != 0){ // sprawdzamy czy jestesmy polaczeni z baza danych
        ...
        if(!empty($expense_category_id)){
            foreach($expense_category_id as $value){
                ...
                $query_result8 = $connection->query("SELECT SUM(amount) AS expenses_categories_total_amount FROM expenses WHERE user_id='$user_id' AND expense_category_assigned_to_user_id='$value' AND MONTH(date_of_expense)=MONTH(CURDATE()) AND YEAR(date_of_expense)=YEAR(CURDATE())");
