...
try{
    ...
    if($connection->connect_errno != 0){ // sprawdzamy czy jestesmy polaczeni z baza danych
        ...
        $query_result = $connection->query("SELECT SUM(amount) AS total_incomes FROM incomes WHERE user_id='$user_id' AND date_of_income BETWEEN '$date_from' AND '$date_to'");
        $query_result2 = $connection->query("SELECT SUM(amount) AS total_expenses FROM expenses WHERE user_id='$user_id' AND date_of_expense BETWEEN '$date_from' AND '$date_to'");
        $query_result3 = $connection->query("SELECT DISTINCT income_category_assigned_to_user_id AS incomes_categories_id FROM incomes WHERE user_id='$user_id' AND date_of_income BETWEEN '$date_from' AND '$date_to'");
        $query_result4 = $connection->query("SELECT DISTINCT expense_category_assigned_to_user_id AS expenses_categories_id FROM expenses WHERE user_id='$user_id' AND date_of_expense BETWEEN '$date_from' AND '$date_to'");
        ...
        if(!empty($income_category_id)){
            foreach($income_category_id as $value){
                ...
                $query_result6 = $connection->query("SELECT SUM(amount) AS incomes_categories_total_amount FROM incomes WHERE user_id='$user_id' AND income_category_assigned_to_user_id='$value' AND date_of_income BETWEEN '$date_from' AND '$date_to'");
        ...
        if(!empty($expense_category_id)){
            foreach($expense_category_id as $value){
                ...
                $query_result8 = $connection->query("SELECT SUM(amount) AS expenses_categories_total_amount FROM expenses WHERE user_id='$user_id' AND expense_category_assigned_to_user_id='$value' AND date_of_expense BETWEEN '$date_from' AND '$date_to'");
...
<head>
    ...
    <link rel="stylesheet" href="Personal_Budget_NONSTANDARD.css">
</head>
